  ...
  if (load === 'error')
    return (
      <div style={{ textAlign: 'center', padding: '54px 16px', color: '#8a85a0' }}>
        ...
        <button onClick={refresh} style={{ border: 'none', borderRadius: 10, padding: '9px 18px', fontWeight: 700, fontSize: 13, color: '#fff', background: brandGradient, cursor: 'pointer' }}>{t.retry}</button>
      </div>
    );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, alignItems: 'center' }}>
        ...
        <button onClick={() => setPanel({ mode: 'create' })} style={{ marginLeft: 'auto', display: 'inline-flex', alignItems: 'center', gap: 8, border: 'none', borderRadius: 11, padding: '10px 18px', fontSize: 14, fontWeight: 700, color: '#fff', background: brandGradient, cursor: 'pointer' }}>
          <Icon path="M12 5v14M5 12h14" size={17} stroke="#fff" />{t.bpCreate}
        </button>
      </div>
      ...
      {profiles.length === 0 ? (
        ...
        <div style={{ textAlign: 'center', padding: '40px 16px', color: '#8a85a0', fontSize: 14 }}>{t.listEmpty}</div>
      ) : (
  ...
  return (
    <div style={{ textAlign: 'center', padding: '64px 16px' }}>
      <div style={{ width: 72, height: 72, borderRadius: 20, background: 'linear-gradient(150deg,#f6f2ff,#fcf1fc)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 18px' }}>
        <Icon path="M12 2l2.6 5.3 5.9.9-4.3 4.1 1 5.8L12 21l-5.2 2.8 1-5.8L3.5 8.2l5.9-.9z" size={32} stroke="#a78bfa" />
      </div>
      ...
      <button onClick={onCreate} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, border: 'none', borderRadius: 12, padding: '12px 24px', fontSize: 14, fontWeight: 700, color: '#fff', background: brandGradient, cursor: 'pointer' }}>
        <Icon path="M12 5v14M5 12h14" size={17} stroke="#fff" />{t.bpCreateFirst}
      </button>
    </div>
  );
}
