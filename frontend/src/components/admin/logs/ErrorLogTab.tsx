...
import AdminListPage, { SearchInput, FilterSelect, DataTable, DetailRow, type ListState } from '../AdminListPage';
import { usePageSize } from '../../../hooks/usePageSize';
import { getSystemLogs, logLevelTone, type SystemLog, type LogLevel } from '../../../api/admin';

// Tab A "Log lỗi hệ thống" — chuyển nguyên từ pages/admin/Logs.tsx, GIỮ NGUYÊN hành vi
// (tìm kiếm, lọc mức độ, lọc ngày, gom nhóm trùng, phân trang server-side).
// Hai thay đổi duy nhất: số dòng/trang giờ chọn được (trước cố định 12) và `page` đổi sang
// 0-based cho khớp 4 trang quản trị còn lại.
...
export default function ErrorLogTab() {
  ...
  const [pageSize, setPageSize] = usePageSize('logs-error');
  ...
  }, [query, level, date, grouped, page, pageSize, reload]);

  useEffect(() => setPage(0), [query, level, date, grouped, pageSize]);
