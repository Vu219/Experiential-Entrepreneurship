...
public class ContentFormattingServiceImpl implements ContentFormattingService {
    ...
    public ApiResponse<ContentFormattingJobResponse> startFormatting(String email, UUID itemId,
        ...
        return ApiResponse.success("Đã bắt đầu định dạng nội dung theo nền tảng",
                contentFormattingMapper.toJobResponse(saved, currentVersions(item)));
    }
    ...
    public ApiResponse<ContentFormattingJobResponse> getJob(String email, UUID jobId) {
        ...
        return ApiResponse.success("Lấy trạng thái tác vụ định dạng nội dung thành công",
                contentFormattingMapper.toJobResponse(job, currentVersions(job.getContentItem())));
    }
